...
def whats_new(session):
    """
    Парсер выводит список на статьи о нововведениях
    в Python и справочную информацию.
    """
    ...
    for section in tqdm(sections_by_python, colour='magenta'):
...
def latest_versions(session):
    """Парсер скачивает архив с документацией Python."""
...
def download(session):
    """Парсер скачивает архив с документацией Python на локальный диск."""
...
def pep(session):
    """
    Парсер выводит информацию о количестве PEP
    ...
    for tr_tag in tqdm(tr_tags, colour='magenta'):
        ...
        dt_tag = dl_tag.find('dt', string='Status')
        ...
        try:
            if status_tag not in EXPECTED_STATUS[preview_status]:
                if (len(preview_status) > 2 or
                        EXPECTED_STATUS[preview_status] is None):
    ...
    results.append(('Total: ', total_pep_sum))
    return results
